...
# Median home price data at the county level.
# Filter to just get the counties in Portland.
# Make the data long.
# Create the diff variable that captures the change in house price by year.
...
# Clean the dataset.
# Rename variables.
# Create total variable which is the sum of all the different permit 
# classifications.
# Use gather to make the data long.
...
oregon_county_pop <- read_csv("data/oregon_county_pop.csv", 
                              col_types = cols(
...
# Population data from 2000 - 2009
# Clean the data to just get the columns I want.
# Filter to just include the counties in Portland.

oregon_county_pop_2000 <- read_csv("data/counties_2000_2009.csv", 
                                   col_types = cols(
                                                    .default = col_double(),
...
# For some reason it wasn't working to have it all as one piped function so I
# separated them out to turn the wide data long with gather.

oregon_county_pop <- oregon_county_pop %>%
  gather(year, pop, 3:21)

# Merge everything into one dataset.
# House price data, permit data, combined population data
...
# Write the unified dataset as a csv and save it to the shiny app folder.
...
# Create the animated plot using the neighborhoods data.
# Standard ggplot.
# Use transition_time() to animate.
# Save plot as a rds file to the shiny app folder.
